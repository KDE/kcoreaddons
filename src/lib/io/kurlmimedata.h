 ...
 *
 * \brief Utility functions for using URLs in QMimeData.
 *
 * In addition to QMimeData::setUrls() and QMimeData::urls(), these functions allow to:
 * \list
 * \li Store two sets of URLs, the KDE-specific URLs and the equivalent local-file URLs
 *     for compatibility with non-KDE applications
 * \li Store KIO metadata, such as the HTTP referrer for a given URL (some websites
...
/*!
 * \typedef KUrlMimeData::MetaDataMap
 *
 * Alias for QMap<QString, QString>
 */
 ...
 *
 * \a mimeData the QMimeData instance used to drag or copy this URL
 */
...
/*!
 * \a metaData KIO metadata shipped in the mime data, which is used for instance to
...
/*!
 * Extract a list of urls from the contents of \a mimeData.
 *
 ...
 *
 * Decoding will fail if \a mimeData does not contain any URLs, or if at
 ...
 *
 * \a decodeOptions options for decoding
 *
 * \a metaData optional pointer to a map which will hold the metadata after this call
 *
