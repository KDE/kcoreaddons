...
{
    const QStringView strView{staleFileName};
    ...
    const auto sep = strView.right(3);
    const int sepPos = staleFileName.indexOf(sep);
    const QByteArray managedFilename = strView.left(sepPos).toLatin1();
    ...
    const QByteArray encodedPath = strView.mid(pathPos + 1, staleFileName.length() - pathPos - 1 - KAutoSaveFilePrivate::NamePadding).toLatin1();
...
bool staleMatchesManaged(const QString &staleFileName, const QUrl &managedFile)
{
    ...
    if (managedFile.fileName() != QUrl::fromPercentEncoding(strView.left(sepPos).toLatin1())) {
        return false;
    }
    // Check paths
    const int pathPos = staleFileName.indexOf(QChar::fromLatin1('_'), sepPos);
